defmodule FrontendExWeb.TxController do
  ...
  def show(conn, %{"hash" => hash}) when is_binary(hash) do
    ...
    tx_task = Task.async(fn -> Client.get_json_cached("/api/v2/transactions/#{hash}", :public) end)
    logs_task = Task.async(fn -> Client.get_json_cached("/api/v2/transactions/#{hash}/logs", :public) end)
    ...
    tx_json = await_ok(tx_task)
    logs_json = await_ok(logs_task)
    blocks_json = await_ok(blocks_task)
    ...
    if is_nil(tx_json) do
      ...
      gas_price_eth = if display_tx.gas_price, do: Format.format_wei_to_eth_exact(display_tx.gas_price), else: nil
      gas_price_gwei = if display_tx.gas_price, do: Format.format_wei_to_gwei(display_tx.gas_price), else: nil
  ...
  defp address_flags(%{} = tx) do
    ...
    from_is_contract_like = truthy?(from_info && from_info["is_contract"]) || from_is_verified || is_binary(from_info && from_info["name"])

    to_is_verified = truthy?(to_info && to_info["is_verified"])
    to_is_contract_like = truthy?(to_info && to_info["is_contract"]) || to_is_verified || is_binary(to_info && to_info["name"])
