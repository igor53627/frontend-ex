defmodule FrontendExWeb.TokensParityTest do
  ...
  @golden_classic_path Path.expand("../golden/tokens.classic.rust.html", __DIR__)
  @golden_53627_path Path.expand("../golden/tokens.53627.rust.html", __DIR__)

  @frozen_now ~U[2026-02-09 12:00:00Z]
  @fixture_api_url "http://127.0.0.1:4901"
  @explorer_url "https://sepolia.53627.org"

  test "classic /tokens matches Rust HTML byte-for-byte" do
    restore =
      put_env(%{
        ff_skin: "classic",
    ...
    body = html_response(get(build_conn(), "/tokens"), 200)

    Golden.assert_golden!(@golden_classic_path, body)
  end

  test "53627 /tokens matches Rust HTML byte-for-byte" do
    restore =
      put_env(%{
        ff_skin: "53627",
        blockscout_url: @explorer_url,
