defmodule FrontendExWeb.HomeParityTest do
  use FrontendExWeb.ConnCase, async: true

  alias FrontendEx.TestSupport.Golden

  @golden_classic_path Path.expand("../golden/home.classic.rust.html", __DIR__)
  @golden_53627_path Path.expand("../golden/home.53627.rust.html", __DIR__)

  # The Rust goldens were generated from API fixtures whose timestamps are in late 2025.
  ...
  test "classic / matches Rust HTML byte-for-byte" do
    restore =
      put_env(%{
        ff_skin: "classic",
    ...
    body = html_response(get(build_conn(), "/"), 200)

    Golden.assert_golden!(@golden_classic_path, body)
  end

  test "53627 / matches Rust HTML byte-for-byte" do
    restore =
      put_env(%{
        ff_skin: "53627",
        blockscout_url: @explorer_url,
